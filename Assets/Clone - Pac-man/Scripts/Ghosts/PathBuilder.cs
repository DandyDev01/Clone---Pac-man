...
public class PathBuilder
{
	...
	public List<Node> CalculatePath(Vector2 target, Vector2 start)
	{
		...
		start = _grid.GetCellPosition(start);
		start = _grid.GetWorldPosition((int)start.x, (int)start.y);

		// normalize the target location to the grid coords (alignment)
		target = _grid.GetCellPosition(target);
		target = _grid.GetWorldPosition((int)target.x, (int)target.y);
