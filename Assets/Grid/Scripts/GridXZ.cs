...
namespace Grid
{
    public class GridXZ<TGridObject> : Grid<TGridObject>
    {
		public GridXZ(Vector3 _oragin, int _cols, int _rows, float _cellSize) : base(_oragin, _cols, _rows, _cellSize)
		{
		}
		...
		public override Vector3 GetCellPosition(Vector3 worldPosition)
		{
			//Vector2 vector = new Vector2(((worldPosition.x / cellSize) - cellSize / 2), ((worldPosition.y / cellSize) - cellSize / 2)) - oragin;
			Vector3 vector = new Vector3(((worldPosition.x / cellSize)), 0,((worldPosition.z / cellSize))) - oragin;
		...
		public override Vector3 GetWorldPosition(int col, int row)
		{
			...
			return new Vector3((col * cellSize + cellSize / 2), 0, (row * cellSize + cellSize / 2)) + oragin;
		}
	}
}
